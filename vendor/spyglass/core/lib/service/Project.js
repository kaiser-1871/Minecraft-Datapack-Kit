...
export class Project extends EventDispatcher {
    ...
    static AnalysisYieldInterval = 100;
    /**
    ...
    async analyzeProject(options = {}) {
        ...
        const files = [...new Set(this.getTrackedFiles().map((uri) => this.normalizeUri(uri)))]
            .filter((uri) => this.projectRoots.some((root) => fileUtil.isSubUriOf(uri, root))
        ...
        for (const uri of files) {
            if (options.signal?.aborted) {
                cancelled = true;
                this.logger.info(`[Project#analyzeProject] Cancelled after ${done}/${files.length} files`);
                break;
            }
            try {
                if (this.#clientManagedUris.has(uri)) {
...
], Project.prototype, "analyzeProject", null);
__decorate([
    SingletonPromise()
