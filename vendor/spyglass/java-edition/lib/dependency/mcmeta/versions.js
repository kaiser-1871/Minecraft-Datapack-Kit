...
import { McmetaVersions, MojangClientJson, MojangVersionJson, MojangVersionManifest, } from './types.js';
const SPYGLASSMC_API_URI = 'https://api.spyglassmc.com/mcje/versions';
const GITHUB_API_URI = 'https://raw.githubusercontent.com/misode/mcmeta/refs/heads/summary/versions/data.min.json';
const MOJANG_VERSION_MANIFEST_URI = 'https://piston-meta.mojang.com/mc/game/version_manifest_v2.json';
const MAX_CLIENT_JAR_FETCHES = 5;
export async function fetchMcmetaVersions(externals, logger, { __loadBundledOverrideForTestOnlyDoNotUseOrYouGetFired } = {}) {
    const candidates = [];
    ...
    return hydrateWithMojangApi(externals, logger, reduceCandidates(candidates), mojangVersionManifest);
}
/**
 * Return the McmetaVersions if it is fresh; append it in `candidates` argument if stale; no-op if error.
 */
async function fetchRemoteMcmetaVersions({ externals, logger, candidates, uri, }) {
    let versions;
    try {
 ...
 */
async function loadBundledMcmetaVersions({ logger, }) {
    let result;
    try {
 ...
 */
async function hydrateWithMojangApi(externals, logger, versions, mojangVersionManifest) {
    if (!mojangVersionManifest) {
        return versions;
    }
    const existingMcmetaVersions = new Set(versions.map((v) => v.id));
    const newMojangVersionManifestEntries = [];
    ...
    const newMcmetaVersions = await Promise.all(newMojangVersionManifestEntries.map((mojangVersionManifestEntry, index) => inferMcmetaVersionFromMojangVersionManifestEntry({
        externals,
        logger,
    ...
    return newMcmetaVersions.concat(versions);
}
async function inferMcmetaVersionFromMojangVersionManifestEntry({ externals, logger, mojangVersionManifestEntry, latestMcmetaVersion, index }) {
...
async function fetchMojangVersionJsonFromClientJar({ externals, logger, clientJarUri, index }) {
    if (index >= MAX_CLIENT_JAR_FETCHES) {
        logger.info(`Skip fetching version.json from client Jar ${clientJarUri} as ${index} >= ${MAX_CLIENT_JAR_FETCHES}`);
        return undefined;
    }
    let versionJson;
    try {
        const zipReader = new ZipReader(new HttpReader(clientJarUri, {
