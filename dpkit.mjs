#!/usr/bin/env node
// dpkit.mjs — Datapack Kit: drive the Spyglass language server (the engine behind
// "Datapack Helper Plus") over LSP stdio to check a datapack AND teach its exact
// per-version command syntax to humans or AIs.
//
// Usage:
//   node dpkit.mjs                                             # check the pvp datapack as version 26.2
//   node dpkit.mjs --version=auto                              # let the server pick from pack.mcmeta
//   node dpkit.mjs --datapack=D:\path\to\pack                 # check a different datapack
//   node dpkit.mjs --files=battle/function/snowman/*.mcfunction # only these (data-relative, * glob)
//   node dpkit.mjs --json                                      # machine-readable JSON report
//   node dpkit.mjs --delta                                     # only re-report files whose issues changed
//   node dpkit.mjs --no-ignore                                 # stop filtering known false positives
//   node dpkit.mjs --ignore='Unknown key "Foo"'                # extra ignore pattern (repeatable, /regex/)
//   node dpkit.mjs --verbose                                   # also print server's own log lines
//   node dpkit.mjs --no-gotchas --no-log                       # disable gotcha linter / game-log self-check
//   node dpkit.mjs --syntax="execute on"                       # offline: grammar of a command path
//   node dpkit.mjs --complete=file:line:col                    # live completion at a cursor
//
...
import { loadCommandTree, loadCachedVersions, cachedCommandVersions, renderPath, renderAll } from './syntax.mjs';

const TOOLS_DIR = fileURLToPath(new URL('.', import.meta.url));
const SERVER = join(TOOLS_DIR, 'node_modules', '@spyglassmc', 'language-server', 'bin', 'server.js');
const LEGACY_DEFAULT_DATAPACK = join(TOOLS_DIR, '..', 'datapacks', 'pvp'); // 工具挪位置后失效的相对默认路径
...
const BASELINE_FILE = join(TOOLS_DIR, '.dpkit-baseline.json');
...
// Suppressed from the report by default; disable with --no-ignore. --ignore=<substring|/regex/>
// appends extra patterns. Ignored diagnostics never count toward the exit code.
// Note: the server renders the key name with typographic quotes (“ ”), not ASCII ones.
...
const extraIgnores = process.argv
  ...
  .map(parsePattern);
const ignorePatterns = USE_IGNORE ? [...BUILTIN_IGNORE_PATTERNS, ...extraIgnores] : extraIgnores;
const matchesIgnore = (msg) => ignorePatterns.some(p => p instanceof RegExp ? p.test(msg) : msg.includes(p));

// Progress/startup lines must not pollute stdout in --json mode (stdout carries pure JSON).
const out = (msg) => JSON_OUT ? console.error(msg) : console.log(msg);
...
// These run against the cached per-version command tree so the AI can learn the
// exact grammar of a command ("what can follow `execute on`?") in milliseconds.
async function printVersions() {
  let list = null, src = '本地缓存';
  try {
    const res = await fetch('https://api.spyglassmc.com/mcje/versions', { signal: AbortSignal.timeout(6000) });
    if (res.ok) { list = await res.json(); src = '服务器(在线)'; }
  ...
  const newer = (isPinned && latestRelease && latestRelease.id !== configured)
    ...
    : null;
  const rows = list.slice(0, 14).map(v => ({
    id: v.id, type: v.type ?? '?', dpv: v.data_pack_version, hasData: cached.has(v.id),
  }));
  ...
  if (JSON_OUT) {
    console.log(JSON.stringify({
      ...
      latestSnapshot: latestSnap ? { id: latestSnap.id, data_pack_version: latestSnap.data_pack_version } : null,
  ...
  out(`可用版本(来自 ${src}, 共 ${list.length} 个):`);
  out(`  最新正式版: ${latestRelease?.id}  (data_pack_version ${latestRelease?.data_pack_version})${cached.has(latestRelease?.id) ? '  ✓数据已缓存' : '  数据未缓存,首次用需下载'}`);
  ...
  for (const r of rows) out(`    ${r.id.padEnd(18)} ${(r.type ?? '').padEnd(8)} dpv ${String(r.dpv).padEnd(4)} ${r.hasData ? '✓' : '—'}`);
  if (list.length > rows.length) out(`    …(共 ${list.length} 个, 只显示最近 ${rows.length} 个)`);
  ...
  await (async () => {
    try {
      ...
      if (DUMP_GIVEN || DUMP_ALL) {
        const target = DUMP || join(TOOLS_DIR, `command-reference-${GAME_VERSION}.md`);
      ...
      if (JSON_OUT) {
        // keep stdout pure machine-readable even in --json mode
      ...
      console.error(`[check] --syntax/--dump 失败: ${err.message}`);
      process.exit(2);
    }
  })();
}
...
const normUri = (uri) => uri.replace(/^file:\/\/\/[A-Z]:\//, m => m.toLowerCase());

// ---------- collect files ----------
// Paths are made relative to data/ so --files=battle/... reads naturally (matches README).
...
const diagnostics = new Map(); // normUri -> array of diagnostic (last wins)
const checkFailed = new Set(); // normUri -> server threw during check (no diagnostics emitted)
const opened = new Set(); // normUri
let ready = false;
let serverLog = [];
...
// 引擎的宽松 schema 对"未知键/错层级"保持沉默、对运行时行为一无所知——这些正则捕获包内已知的
// 静默失败写法(见 CLAUDE.md 版本要点)。纯尽力而为:只报警告,绝不影响退出码。
const lineOf = (text, needle, from = 0) => {
  const i = text.indexOf(needle, from);
  return i < 0 ? null : text.slice(0, i).split('\n').length;
};

function gotchaScan(f) {
    ...
    try {
      ...
      const walk = (v) => {
        ...
        if (v && typeof v === 'object') {
          if ('criteria' in v && typeof v.criteria === 'object' && v.criteria !== null && Array.isArray(v.requirements) && v.requirements.length >= 2) {
            const trig = Object.values(v.criteria).map(c => c?.trigger).filter(Boolean);
            const dup = trig.length !== new Set(trig).size;
            const or = v.requirements.some(g => Array.isArray(g) && g.length <= 1);
...
function report() {
  ...
  const lines = [];
  const byMessage = new Map(); // message -> count (non-ignored only)
  const ignoredByMessage = new Map(); // message -> count
  const issues = []; // for --json
  const ignoredList = []; // for --json

  // 26.2 已知坑扫描(heuristic):引擎宽松 schema 漏掉、游戏里却静默失败的写法
  const gotchaByFile = new Map(); // rel -> [{line,key,msg}]
  let gotchaCount = 0;
  if (!NO_GOTCHAS) {
    for (const f of files) {
  ...
  const glog = LOGCHECK ? gameLogReport() : { found: false };

  let baseline = {};
  const newBaseline = { datapack: DATAPACK, version: GAME_VERSION, files: {} };
  if (DELTA) {
    try {
  ...
  for (const f of files) {
    ...
    if (!ds) {
      // No diagnostics arrived at all: either the server threw (logged) or it silently blocked.
  ...
  if (DELTA) {
    ...
    catch (err) { console.error(`[check] could not write baseline ${BASELINE_FILE}: ${err.message}`); }
  }
  ...
  // Only nag when the effective version is strictly older than the newest release —
  // not when the user is already on a newer snapshot.
  try {
    const vl = loadCachedVersions();
    if (vl?.length) {
      const latest = vl.filter(v => v.type === 'release')[0];
      const effective = resolvedVersion ?? GAME_VERSION;
      const eff = vl.find(v => v.id === effective);
...
const KIND_NAMES = { 1:'文本',2:'方法',3:'函数',4:'构造',5:'字段',6:'变量',7:'类',8:'接口',9:'模块',10:'属性',11:'单位',12:'值',13:'枚举',14:'关键字',15:'片段',16:'颜色',17:'文件',18:'引用',19:'文件夹',20:'枚举成员',21:'常量',22:'结构',23:'事件',24:'操作符',25:'类型参数' };
function completionItemsOf(res) {
  return (Array.isArray(res) ? res : (res?.items ?? [])).map(it => ({
    label: it.label,
    kind: KIND_NAMES[it.kind] ?? `kind${it.kind}`,
    detail: it.detail ?? null,
    documentation: (typeof it.documentation === 'object' ? (it.documentation?.value ?? '') : (it.documentation ?? '') || '') || null,
  }));
}
function printCompletion(rel, ln, col, res) {
  ...
  try {
    ...
    if (COMPLETE) {
      ...
      try {
        ...
        console.error(`[check] completion 查询失败: ${err?.message ?? err}`);
        finish(2);
      }
      return;
    }
    ...
    console.error(`[check] internal failure: ${err?.stack ?? err}`);
    finish(2);
  }
})();
