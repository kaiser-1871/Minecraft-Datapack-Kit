...
export const argument = (rawTreeNode, prevNodes) => {
    ...
    switch (treeNode.parser) {
        ...
        case 'minecraft:team_color':
            return wrap(core.map(commandLiteral({ pool: core.Color.ColorNames }), (res) => ({
                ...res,
                color: core.Color.NamedColors.has(res.value)
                    ? core.Color.fromCompositeRGB(core.Color.NamedColors.get(res.value))
                    : undefined,
...
function time(minimum = 0) {
    return core.validate(core.map(core.sequence([
        float(),
